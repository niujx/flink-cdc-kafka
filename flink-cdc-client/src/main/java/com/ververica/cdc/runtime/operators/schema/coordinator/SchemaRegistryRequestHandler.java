...
public class SchemaRegistryRequestHandler {
    ...
    public CompletableFuture<CoordinationResponse> handleSchemaChangeRequest(
        ...
        if (pendingSchemaChanges.isEmpty()) {
            ...
            boolean  isMongodb = request.getSchemaChangeEvent() instanceof CreateTableEvent &&
                    StringUtils.equals(((CreateTableEvent)request.getSchemaChangeEvent()).getSchema().comment() ,"mongodb");
            ...
            //为了在同步mongodb情况每次都能发送，createTableEvent 修改了schema 注册的流程，mongodb情况下跳过以注册检查
            if(isMongodb && !schemaManager.schemaExists(request.getTableId())) {
