...
public class TiKVRichParallelSourceFunction<T> extends RichParallelSourceFunction<T>
    ...
    private final StartupOptions startupMode;
    ...
    public void open(final Configuration config) throws Exception {
        ...
        resolvedTs =
                startupMode.startupMode == StartupMode.INITIAL
    ...
    public void run(final SourceContext<T> ctx) throws Exception {
        ...
        if (startupMode.startupMode == StartupMode.INITIAL) {
            synchronized (sourceContext.getCheckpointLock()) {
                readSnapshotEvents();
            }
        }
        if (startupMode.startupMode == StartupMode.TIMESTAMP) {
            resolvedTs = new TiTimestamp(startupMode.timestamp, 0).getVersion();
        } else {
